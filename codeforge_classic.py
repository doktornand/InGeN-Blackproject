...
# ═══════════════════════════════════════════════════════════════════
#  SYNTAX HIGHLIGHTER (Modern Colors)
# ═══════════════════════════════════════════════════════════════════
class CodeHighlighter(QSyntaxHighlighter):
    ...
    def _build_rules(self):
        self.rules = []
        kw_cs = r'\b(public|private|static|void|string|int|bool|var|new|return|if|else|for|foreach|while|try|catch|throw|using|class|namespace|async|await|true|false|null|this|let|const)\b'
...
class CanvasWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        f = QFont('Segoe UI', 9)
        painter.setFont(f)
        painter.drawText(12, 24, "CODEFORGE v2.0 — NODAL CANVAS")
...
class CodeGenerator:
    ...
    def _gen_csharp_v2(ordered, fns, vars_, ctrls, groups, data_src, flow_next, name, wrap_class):
        ...
        lines = [f"// Generated by CODEFORGE v2.0 [NODAL] - {ts}", "", "using System;", "using System.IO;", "using System.Collections.Generic;", ""]
    ...
    def _gen_ps_v2(ordered, fns, vars_, ctrls, groups, data_src, flow_next, name):
        ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        lines = [f"# Generated by CODEFORGE v2.0 [NODAL] - {ts}", "#Requires -Version 5.1", "Set-StrictMode -Version Latest", "$ErrorActionPreference = 'Stop'", ""]
        if vars_:
    ...
    def _gen_js_v2(ordered, fns, vars_, ctrls, groups, data_src, flow_next, name):
        ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        lines = [f"// Generated by CODEFORGE v2.0 [NODAL] - {ts}", "'use strict';", ""]
        if vars_:
            lines.append("// ─── Variables ───────────────────────────────────")
            for b in vars_:
                d = b.data
...
class CodeForgeMainWindow(QMainWindow):
    ...
    def _setup_statusbar(self):
        ...
        self.status_main = QLabel("CodeForge v2.0 — Nodal Mode")
