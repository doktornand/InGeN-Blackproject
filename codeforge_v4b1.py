...
class CodeHighlighter(QSyntaxHighlighter):
    ...
    def _build_rules(self):
        ...
        self.rules += [
            ...
            (re.compile(r"'[^'\\]*(?:\\.[^'\\]*)*'"), self._fmt(C['amber'])),
            (re.compile(r'\d+(\.\d+)?'),     self._fmt(C['amber_bright'])),
            (re.compile(r'//[^\\n]*'),            self._fmt(C['comment'], italic=True)),
            (re.compile(r'#[^\\n]*'),             self._fmt(C['comment'], italic=True)),
            (re.compile(r'[A-Z][a-zA-Z0-9_]*(?=\()'), self._fmt(C['green_bright'])),
        ...
        if self.language == 'Python':
            ...
            self.rules.append((re.compile(r'\d+\.?\d*'), self._fmt(C['amber_bright'])))
...
class CanvasBlock(QFrame):
    ...
    def paintEvent(self, event):
        ...
        for port in self.ports:
            ...
            # FIX: Wrapped color strings in QColor() to prevent QPen TypeError
    ...
    def set_selected(self, selected):
        ...
        if selected:
            ...
            glow.setColor(QColor(0, 0, 0, 80)) # Soft modern shadow
            glow.setOffset(0, 4)
...
class CanvasWidget(QWidget):
    ...
    def ensure_visible(self, pos, margin=200):
        """Ajuste la taille du canvas si un bloc sort des limites visibles."""
        ...
        if changed:
            self.update()
            # Propager la mise à jour au parent container pour le scroll area
    ...
    def _do_add_block(self, data, block_type, pos):
        ...
        # ─── FIX SCROLL: étendre le canvas si le bloc est placé loin ───
    ...
    def auto_layout(self):
        ...
        # ─── FIX SCROLL: recalculer les limites après layout ───
        if self.blocks:
            ...
            if b.block_type == CanvasBlock.BLOCK_VARIABLE: continue
                ...
                for p in d.get('parameters', []):
                    ...
                    if port and port in data_src:
                        ...
                        args_list.append(p.get('default', '$null') if not p.get('required') else '$null')
                arg_str = ' '.join(f"-{p.get('name')} {a}" for p, a in zip(d.get('parameters', []), args_list)) if d.get('parameters') else ''
                lines.append(f"{d.get('name', 'Func')} {arg_str}")
            elif b.block_type == CanvasBlock.BLOCK_CONTROL:
...
class CodeForgeMainWindow(QMainWindow):
    ...
    def _setup_central(self):
        ...
        # ─── SCROLL AREA AVEC BARRES TOUJOURS VISIBLES ───
